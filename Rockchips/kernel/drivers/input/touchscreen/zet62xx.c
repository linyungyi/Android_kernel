...
#define CONFIG_RK3026_BOARD_AK47_V5_2
    ...
    //#include "zet6223_fw_rk3026_w9.h"	 
	 //#include "zet6223_fw_mf_20140916.h"
	 #include "C101_aikeou_zet6223_fw_20140530.h"
    #include "zet6221_fw.h"
   // #include "zet6251_fw.h"
    #include "f788S_ZET6251_FW.h"
    #include "zet6231_fw.h"
    ...
    #include "zet6221_fw.h"
    //#include "zet6223_fw.h"
	#include "ZET6223_FW_jihexing_20140313.h"
    #include "zet6231_fw.h"
...
//	#define FEATURE_FW_COMPARE		///< 
	#define FEATURE_FW_UPGRADE_RESUME	///<
		#define FEATURE_FW_CHECK_SUM       ///< ZET6251 
...
///---------------------------------------------------------------------------------///
//#define FEATURE_CHECK_LAST_PAGE 		///< 

///---------------------------------------------------------------------------------///
///  10. Dummy report (without pull high resistor)
///---------------------------------------------------------------------------------///
//#define FEATURE_DUMMY_REPORT			///< 
...
extern int axp_set_gpio_io(int gpio, int io_state);
extern int axp_set_gpio_value(int gpio, int value);
#endif
///---------------------------------------------------------------------------------///
///  16. IOCTRL Debug
///---------------------------------------------------------------------------------///
...
void zet622x_ts_coordinate_translating(struct finger_coordinate_struct* finger_report_data)
{
	...
	for(i = 0 ; i < MAX_FINGER_NUMBER ; i++)
	{
		if(finger_report_data[i].valid == TRUE)
		{
			finger_report_data[i].report_x  = finger_report_data[i].report_x ;
		}
	}
	...
	for(i = 0 ; i < MAX_FINGER_NUMBER ; i++)
	{
		...
		{
			finger_report_data[i].report_x  = finger_report_data[i].report_x ;
			finger_report_data[i].report_y =  finger_report_data[i].report_y;
		}
	}
...
int zet622x_ts_sig_write(struct i2c_client *client)
{
	...
	///---------------------------------///
        /// if zet6221, zet6251 zet6270 then leaves
        ///---------------------------------///
	if(ic_model == MODEL_ZET6221 || ic_model == MODEL_ZET7130 || ic_model == MODEL_ZET7150)
	{
...
LABEL_RESUME_END:	
	...
	if(checksum_diff == 1)
	{
		pr_err("[ZET]: checksum diff ,download fw !\n");
		resume_download_task = kthread_create(zet622x_resume_download_thread, NULL, "resume_download");
		if(IS_ERR(resume_download_task))
		{
			pr_err(KERN_ERR "%s: cread thread failed\n",__FILE__);
		}
		wake_up_process(resume_download_task);
	}
	#endif
...
static void zet_file_save(char *file_name, int data_total_len)
{
	...
	//albert+
	pr_err("[ZET] : %s data_len=%d\n",file_name,data_total_len);
	for(i=0;i<data_total_len;i++)
		printk("%2X ",tran_data[i]);
	pr_err("\n");
	//
	return;
...
int zet_get_flash_checksum(void)
{
	...
	if(ret <= 0)
	{
		pr_err("[ZET]: flash checksum write B8 cmd error !\n");
		return ret;
	}
	//send c1 cmd
	msleep(1);
	...
	if(ret <= 0)
	{
		pr_err("[ZET]: flash checksum read data error !\n");
		return ret;
	}
	//send c1 cmd
	msleep(1);
...
//	}
	...
	{
  memset(buf, 0x00, 64);
  sprintf(pname, "%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X",
                       pcode[0], pcode[1], pcode[2], pcode[3],
                       pcode[4], pcode[5], pcode[6], pcode[7] );
  zet_fw_set_pcode_name(pname);
...
static int  zet622x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	///------------------------------------------------///
	/*
	result = gpio_request(TS_RST_GPIO, "ts_rst_gpio"); //
	if(result)
	{
	...
	*/
    ...
    if(	gpio_request(x_TS_INT_GPIO,NULL) < 0)
		printk("[ZET]: x_TS_INT_GPIO failed..\n");	
    if(	gpio_request(x_TS_RST_GPIO,NULL) < 0)
		printk("[ZET]: x_TS_RST_GPIO failed..\n");
...
*/	
	...
	zet_fw_set_file_name(FW_FILE_NAME);//.bin
...
void power_off_disable_tp_irq(void)
{
	printk("Enter:%s>>>>>>irq_tp=%d>>>>\n",__func__,irq_tp);
	disable_irq(irq_tp);
}
EXPORT_SYMBOL_GPL(power_off_disable_tp_irq);
///************************************************************************
...
static int  zet622x_module_init(void)
{
	...
	///---------------------------------///
	i2c_add_driver(&zet622x_i2c_driver);//Çý
