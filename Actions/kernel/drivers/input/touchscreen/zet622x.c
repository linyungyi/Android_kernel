...
#define TS_INT_GPIO	        ( IRQ_SIRQ0 )					//RK2928_PIN3_PC7
#define TS_RST_GPIO	        ASOC_GPIO_PORTB(3)			//RK2928_PIN3_PC3
...
/// X, Y Resolution
#define FW_X_RESOLUTION				(960)		///< the FW setting X resolution
#define FW_Y_RESOLUTION				(640)		///< the FW setting Y resolution
#define X_MAX	 				(768)		///< the X resolution of TP AA(Action Area)
#define Y_MAX 					(1024)		///< the Y resolution of TP AA(Action Area)
...
#define DRIVER_VERSION "$Revision: 7 $"
...
///=========================================================================================///
u8 suspend_mode 		= FALSE;
u8 suspend_mode2        = FALSE;
...
#define FW_FILE_NAME 		        "/misc/modules/zet62xx.bin"
...
#define FT5X06_POWER_ID			("sensor28")//("touchPannel_power")
#define FT5X06_POWER_MIN_VOL	(3300000)
#define FT5X06_POWER_MAX_VOL	(3300000)
...
extern int atc260x_get_charge_status(void);

void zet622x_ts_charge_mode_enable(void);
void zet622x_ts_charge_mode_disable(void);

void charger_mode_callback(int charger_on)
{
	printk("[ZET] : CALLBACK TP %d !!!!!!!!!\n",status);
...
static void zet622x_ts_timer_task(unsigned long arg)
{
	...
	atc260x_set_adjust_tp_para(charger_mode_callback);
}
...
static void zet622x_charger_cmd_work(struct work_struct *_work)
{
	...
	//printk("[ZET]:zet622x_charger_cmd_work enter\n");	
...
static int actions_board_hw_init(void)
{
	int err=0;
	...
	if ( !tp_regulator ) {
		printk("\n[ZET]:TP init power failed");
		err = -EINVAL;
	}
	
	//err = gpio_request(TS_RST_GPIO, ZET_TS_ID_NAME); 
	//if (err) {
	//	printk("\nTP gpio_request TS_RST_GPIO failed");
	//}
...
static int actions_board_hw_deinit(void)
{
    if ( tp_regulator )
	{
...
///   [return]:
///************************************************************************
static void zet622x_ts_late_resume_level_1(struct early_suspend *handler)
{	
	int err;
	printk("[ZET] : zet622x_ts_late_resume_level_1 START\n");

	dummy_report_cnt = SKIP_DUMMY_REPORT_COUNT;
	charger_status = 0;
	//ctp_ops.ts_wakeup();
	err = actions_board_hw_init();
  ...
  ///--------------------------------------///
	...
	if(resume_download == TRUE)
	{
	  printk("[ZET] : zet6221_ts_resume resume_download\n");
		return 1;
	}
...
LABEL_RESUME_END:	
	...
	printk("[ZET] : zet622x_ts_late_resume_level_1 END\n");


	return err;
}

static void zet622x_ts_late_resume_level_2 (struct early_suspend *handler)
{	
	int i=0;
	
	printk("[ZET] : zet622x_ts_late_resume_level_2 START\n");
	
	struct zet622x_tsdrv *ts = container_of(handler, struct zet622x_tsdrv, work1);
	enable_irq(ts->irq);
	...
	if(resume_download == TRUE)
	{
	  printk("[ZET] : ts_late_resume resume_download\n");
	  suspend_mode = FALSE;
		return;
	}
...
if(atc260x_get_charge_status() == 1) 
	{
		//printk("[ZET] : atc260x_get_charge_status==1,zet622x_ts_charge_mode_enable enter\n");
		zet622x_ts_charge_mode_enable();
	}
	else
	{
		//printk("[ZET] : atc260x_get_charge_status==0,zet622x_ts_charge_mode_disable enter\n");
		zet622x_ts_charge_mode_disable();
	}
}	

///************************************************************************
///   [function]:  zet622x_ts_early_suspend_level_1
///   [parameters]: early_suspend
///   [return]: void
///************************************************************************
static void zet622x_ts_early_suspend_level_1(struct early_suspend *handler)
{
	struct zet622x_tsdrv *ts = container_of(handler, struct zet622x_tsdrv, work1);
	...
	printk("[ZET]: ------zet622x_ts_early_suspend_level_1 enter ------ \n");
	ret = zet622x_i2c_write_tsdata(this_client, ts_sleep_cmd, 1);
	return;	        
}

///************************************************************************
///   [function]:  zet622x_ts_early_suspend_level_2
///   [parameters]: early_suspend
///   [return]: void
///************************************************************************
static void zet622x_ts_early_suspend_level_2(struct early_suspend *handler)
{
	//printk("[ZET]: ------zet622x_ts_early_suspend_level_2 enter ------ \n");

	actions_board_hw_deinit();
...
static struct i2c_driver zet622x_i2c_driver = 
{
	...
	.suspend   = zet622x_ts_early_suspend_level_2,
	.resume    = zet622x_ts_late_resume_level_1,
...
static int __devinit zet622x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	result = i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_BYTE_DATA);

	if ( !result ) 
	{
        	printk("\n[ZET]:I2c bus dosen't support");
      		result = -EFAULT;
        	goto LABEL_I2C_GET_FAIL;
   	 }
	...
	{
		result = -ENOMEM;
		printk( "[ZET]: ------allocate zet6221_ts start------\n");
		goto LABEL_ALLOC_MEM_FAIL;
  	}
	...
	///-----------------------------------------------///
	msleep(10);
	ctp_wakeup();     // Reset MCU
	
	if(zet622x_ts_get_information(client) <= 0)
	{
		return err;
	}
	if(pcode[3]!=0x8)  // not zeitec ic
	{
		goto LABEL_ALLOC_MEM_FAIL;
	}
	...
	if(result)
	{
		//printk( "[ZET]: ------Register input_dev failed------\n");
		goto LABEL_INPUT_DEV_REG_FAIL;
	}
	...
	zet6221_ts->early_suspend.suspend = zet622x_ts_early_suspend_level_1;
	zet6221_ts->early_suspend.resume = zet622x_ts_late_resume_level_2;
	...
	{
		printk("atc260x_get_charge_status====0\n");
		zet622x_ts_charge_mode_disable();
	}
...
LABEL_INPUT_DEV_REG_FAIL:
	...
	//input_dev = NULL;    //// If input device is null, free it would be a trouble.
	
LABEL_ALLOC_MEM_FAIL:
	///-----------------------------------------------///
...
static int __init zet622x_module_init(void)
{
	...
	printk("[ZET] : zet622x_module_init  \n");
	
	ret = actions_board_hw_init();
	
	pClient = i2c_new_device(adap, &tp_info); // ע²ǲCʨ±¸
	...
	///---------------------------------///
	ret = i2c_add_driver(&zet622x_i2c_driver); //¼Ԕ׺et622xȽ¶¯
	{
		printk("\nadd  zet622x_i2c_driver failed ret=%d\n",ret);
	}
...
{
	//printk( "[ZET]: ------zet622x_module_exit enter------\n");
	i2c_del_driver(&zet622x_i2c_driver);
	//printk( "[ZET]: ------zet622x_i2c_driver del OK------\n");	
