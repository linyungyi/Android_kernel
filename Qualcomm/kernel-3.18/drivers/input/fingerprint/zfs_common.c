...
static const struct chip_struct chip_data[] = {
	{CHIP_ZFS1096, 0x020a, 0, ZFS1096_ROWS, ZFS1096_COLUMNS, ZFS10xx_ADC_GROUP_SIZE, 8000,  16,  0x8000},
	{   CHIP_NONE, 0, 0, 0, 0, 0, 0, 0}
};

/*
const zfs_setup_t setup_default_zfs1096 = {
	.adc_gain			= {0, 0, 1, 1},
	.adc_shift			= {1, 2, 1, 1},
...
size_t zfs_calc_huge_buffer_minsize(zfs_data_t *zfs_dev)
{
	...
	buff_req = (zfs_dev->chip.type == FPC1020_CHIP_NONE) ? buff_min :
						(zfs_dev->chip.pixel_columns *
						zfs_dev->chip.pixel_rows *
...
/* -------------------------------------------------------------------- */
int zfs_check_hw_id(zfs_data_t *zfs_dev /*fpc1020_data_t *fpc1020*/)
{
	int error = 0;
	u16 hardware_id;
	int counter = 0;
...
/* -------------------------------------------------------------------- */
int zfs_setup_defaults(zfs_data_t *zfs_dev /* fpc1020 */)
{
	int error = 0;
/*
...
out_err:
	...
	dev_err(&fpc1020->pdev->dev, "%s FAILED %d\n", __func__, error);
*/
	return error;
}
...
/* -------------------------------------------------------------------- */
const char *zfs_hw_id_text(zfs_data_t *zfs_dev /* fpc1020_data_t *fpc1020 */)
{
...
/* -------------------------------------------------------------------- */
int zfs1096_clear_int(zfs_data_t *zfs_dev,/* fpc1020_data_t *fpc1020 */ int code)
{
	int error = 0;
	struct spi_message msg;
...
/* -------------------------------------------------------------------- */
//int zfs1096_wait_for_irq(zfs_data_t *zfs_dev,/* fpc1020_data_t *fpc1020 */ u8 scan_cmd, u8 scan_mode, int timeout)
...
download:	
	...
	if(zfs_dev->flash_type == ZFS1096_SRAM)
	{
		iFlashRestLen = zfs_dev->chip.flash_size; //28k+32bytes
	}else
	{
		iFlashRestLen = zfs_dev->chip.flash_size - 4096; //sector0~7
	}
