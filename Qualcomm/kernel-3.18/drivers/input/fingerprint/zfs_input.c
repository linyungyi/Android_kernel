...
/* -------------------------------------------------------------------- */
int zfs_input_init(zfs_data_t *zfs_dev /*fpc1020_data_t *fpc1020*/)
{
	int error = 0;
	int i;
	...
	if (!error) {
		...
		set_bit(EV_KEY, zfs_dev->input_dev->evbit);		
		
		// set_bit(ZFS_KEY_BACK, 		zfs_dev->input_dev->keybit);
		// set_bit(ZFS_KEY_MENU, 		zfs_dev->input_dev->keybit);
		// set_bit(ZFS_KEY_HOME,	 	zfs_dev->input_dev->keybit);
		// set_bit(ZFS_KEY_WAKEUP, 	zfs_dev->input_dev->keybit);
		
		for(i=128;i<256;i++) {
			set_bit(i, zfs_dev->input_dev->keybit);
		}		
...
/* -------------------------------------------------------------------- */
void zfs_input_destroy(zfs_data_t *zfs_dev /*fpc1020_data_t *fpc1020*/)
{
	dev_dbg(&zfs_dev->pdev->dev, "%s\n", __func__);
...
/* -------------------------------------------------------------------- */
int zfs_input_enable(zfs_data_t *zfs_dev /*fpc1020_data_t *fpc1020*/, bool enabled)
{
	dev_dbg(&zfs_dev->pdev->dev, "%s\n", __func__);
...
/* -------------------------------------------------------------------- */
int zfs_input_set_event(zfs_data_t *zfs_dev /*fpc1020_data_t *fpc1020*/, int key_code, int on_off)
{
	int error = 0;
	input_report_key(zfs_dev->input_dev, key_code, on_off);
	return error;
}
...
/* -------------------------------------------------------------------- */
int zfs1096_input_task(zfs_data_t *zfs_dev /*fpc1020_data_t *fpc1020*/)
{
	int error = 0;
	u8 key_status[2];
