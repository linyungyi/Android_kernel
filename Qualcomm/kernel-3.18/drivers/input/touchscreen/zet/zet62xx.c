...
#define TS_GPIO_IRQ		        (28)
#define TS_GPIO_RST		        (97)
#define TS_INT_GPIO		        TS_GPIO_IRQ 
#define TS_RST_GPIO		        TS_GPIO_RST
#define _MACH_MSM_TOUCH_H_

///=========================================================================================///
///  15.Charge mode  
///=========================================================================================///
//#define FEATURE_CHARGER_MODE		///< 請修改msm_battery，將充電模式啟動

#ifdef FEATURE_CHARGER_MODE
extern int charger_on;			///< 請在msm_battery中宣告此變量，並在充電時，將之設為1，反之0
...
///---------------------------------------------------------------------------------///
///  18. i2c gpio control (customer)  
///---------------------------------------------------------------------------------///
//#define FEATURE_GPIO_TO_I2C_CTL
...
u8 ioctl_buffer[IOCTL_MAX_BUF_SIZE];
...
{
#ifdef FEATURE_GPIO_TO_I2C_CTL
        gpio_tlmm_config(GPIO_CFG(TS_RST_GPIO, 0,
        				GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP,
        				GPIO_CFG_2MA), GPIO_CFG_ENABLE);
        gpio_set_value(TS_RST_GPIO, 0);
#else ///< for FEATURE_GPIO_TO_I2C_CTL
	gpio_direction_output(TS_RST_GPIO, 0);
#endif ///< for FEATURE_GPIO_TO_I2C_CTL
}
...
s32 zet622x_i2c_read_tsdata(struct i2c_client *client, u8 *data, u8 length)
{
	...
	msg.buf      = data;
#ifdef FEATURE_GPIO_TO_I2C_CTL
	return ts_gpio_i2c_x_read(data, length ,client->addr);
#else ///< for FEATURE_GPIO_TO_I2C_CTL
...
#ifdef FEATURE_GPIO_TO_I2C_CTL
	return ts_gpio_i2c_x_write(data,length,client->addr);
#else ///< for FEATURE_GPIO_TO_I2C_CTL
	return i2c_transfer(client->adapter,&msg, 1);
#endif ///< for FEATURE_GPIO_TO_I2C_CTL
}
...
static long zet_fops_ioctl(struct file *file, unsigned int cmd, unsigned long arg )
{
  u8 __user * user_buf = (u8 __user *) arg;
	...
	u8 *buf = ioctl_buffer;//[IOCTL_MAX_BUF_SIZE];
	int input_data;
	int data_size;
 	...
 	else if(cmd == ZET_IOCTL_CMD_DYNAMIC)
  {
	...
	else if(cmd == ZET_IOCTL_CMD_FW_FILE_PATH_GET)
	{
		...
		printk("[ZET]: zet_ioctl: Get_FW_FILE_NAME = %s\n", buf);
	}
...
static int __devinit zet622x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#ifdef CONFIG_TS_GPIO_I2C
        static const int gpio_scl = 124;
        static const int gpio_sda = 33;
        static const int gpio_rst = 97;
        static const int gpio_int = 28;
        char data[2] = {0,0};
#endif
	...
	///-----------------------------------------------///
#ifdef FEATURE_GPIO_TO_I2C_CTL
	result = gpio_request(TS_INT_GPIO,"tp_irq");
#else ///< for FEATURE_GPIO_TO_I2C_CTL
...
#ifdef CONFIG_TS_GPIO_I2C
                if (ts_gpio_i2c_init(gpio_scl,gpio_sda, gpio_rst, gpio_int))
                {
                        goto LABEL_IRQ_REQUEST_FAIL;
                }
                if(0x76 == ts_dect_type(gpio_scl,gpio_sda,data,1)) 
                {
                        printk("[ZET] : icn83xx use gpio_i2c \n");
                } 
                else
                {
                        goto LABEL_IRQ_REQUEST_FAIL;
	...
	///-----------------------------------------------///
#ifdef FEATURE_GPIO_TO_I2C_CTL
	zet6221_ts->irq = MSM_GPIO_TO_INT(gpio_int);
#else ///< for FEATURE_GPIO_TO_I2C_CTL
