...
#define FW_X_RESOLUTION		(720)		///< the FW setting X resolution
#define FW_Y_RESOLUTION		(1280)		///< the FW setting Y resolution
#define X_MAX	 		LCD_WIDTH		///< X resolution of TP AA(Action Area)
#define Y_MAX 			LCD_HEIGHT		///< Y resolution of TP AA(Action Area)
#define LCM_X_RES 	LCD_WIDTH		// X resolution of LCM
#define LCM_Y_RES 	LCD_HEIGHT		// Y resolution of LCM
...
static DEFINE_MUTEX(i2c_data_mutex);
	...
	{
		zet62xx_i2c_dma_va = (u8*) dma_alloc_coherent( &this_client->dev, 4096, &zet62xx_i2c_dma_pa, GFP_KERNEL);//DMA size 4096 for customer
	...
	static void msg_dma_release(){
		if(zet62xx_i2c_dma_va)
		{
	    	dma_free_coherent(&this_client->dev, 4096, zet62xx_i2c_dma_va, zet62xx_i2c_dma_pa);
	       	zet62xx_i2c_dma_va = NULL;
	       	zet62xx_i2c_dma_pa = NULL;
...
static int zet622x_ts_local_init(void) 
{
  	...
  	TPD_DMESG("Zeitec zetouch I2C Touchscreen Driver (Built %s @ %s)\n", __DATE__, __TIME__);
	...
	//TINNO_TOUCH_TRACK_IDS	5
		...
		//for linux 3.8
		input_set_abs_params(tpd->dev, ABS_MT_TRACKING_ID, 0, (FINGER_NUMBER-1), 0, 0);
	#endif
...
static int __init zet622x_module_init(void)
{
	int ret;
     ...
     printk("[ZET] MediaTek zet touch panel driver init\n");
     i2c_register_board_info(IIC_PORT, &zet6221_i2c_tpd, 1);
	 if(tpd_driver_add(&tpd_device_driver) < 0)
		printk("[ZET] add fts driver failed\n");
