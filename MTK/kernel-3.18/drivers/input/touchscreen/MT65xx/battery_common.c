...
//*****************henry++20140120**********************//
#if 1
//fot tp(zet6221) charger mode 
static kal_bool temp_tp = KAL_FALSE;

static kal_bool charger_plug = KAL_FALSE;
//static kal_bool enablecmd = KAL_FALSE;
//static kal_bool disablecmd = KAL_FALSE;
int charger_on = KAL_FALSE;

//extern void ts_write_charge_enable_cmd();
//extern void ts_write_charge_disable_cmd();
#endif
...
#if 1  //for tp(zet6221) charge mode

	 temp_tp = upmu_is_chr_det();

        if ( temp_tp ) // charger is on
        {
        	charger_on = KAL_TRUE;
			
		battery_xlog_printk(BAT_LOG_CRTI, "[ZET]1969:temp_tp=%d, charger_on=%d\n", temp_tp,charger_on);
		battery_xlog_printk(BAT_LOG_CRTI, "[ZET-battery]:charger_on\n");

           // if ( charger_plug == KAL_FALSE )
           // {
           //     charger_plug = KAL_TRUE;
		// enablecmd =KAL_TRUE;
           // }
        }
        else //charger if off
        {
        	charger_on = KAL_FALSE;
		battery_xlog_printk(BAT_LOG_CRTI, "[ZET]1981:temp_tp=%d, charger_on=%d\n", temp_tp,charger_on);
		battery_xlog_printk(BAT_LOG_CRTI, "[ZET-battery]:charger_off\n");
           // if ( charger_plug == KAL_TRUE )
           // {
            //    charger_plug = KAL_FALSE;
		 // disablecmd =KAL_TRUE;
           // }
        }
